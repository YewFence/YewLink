...
### 本地构建生成器
```bash
cd generator && go build -ldflags="-s -w" -o yewlink-init .
./yewlink-init  # 使用默认参数运行
./yewlink-init -services config.yaml -template config.yaml.tmpl -output config-no-manually-edit.yaml
```
...
1. **yewlink-init 容器**（一次性）：运行 Go 生成器 `yewlink-init`，读取用户的 `config.yaml` + 内置模板 `config.yaml.tmpl`，输出 Infisical CLI 能理解的完整配置到共享 volume
2. **yewlink 容器**（常驻）：使用官方 `infisical/cli` 镜像，以 agent 模式运行，按配置轮询 Infisical API 并将 secrets 写入 `secrets/<服务名>.env`

### 核心文件

- `generator/main.go` — 单文件 Go 程序，包含配置读取、校验、模板渲染全部逻辑。输出的 CLI 名称为 `yewlink-init`
...
- `Dockerfile` — 多阶段构建：Go builder → Alpine 运行时，入口点直接调用 yewlink-init

### 数据流
```
config.yaml (用户编辑) + config.yaml.tmpl (内置模板)
  → yewlink-init 生成器渲染 → Infisical Agent 配置
...
- `docker.yml` — 推送 `v*` tag 时构建并推送 Docker 镜像到 `ghcr.io/yewfence/yewlink-init`
